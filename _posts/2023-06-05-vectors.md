...
If you think of a vector as the difference between two points, you could say it has two properties: a direction and a length or magnitude. When considered on its own, a property it does not have is position. A vector can tell you the direction and distance a point might change by, but only the point can tell you where something is in space.

Just like points, vectors can be added to and subtracted from one another. Vectors can also be multiplied and divided by real numbers, which changes their magnitude. You can think of the vector as being scaled by these real number, so the real numbers are often called scalars.
...
I'll just start with the `struct` declaration:
...
And since I'll want to try different random vectors when I can, I've added this constant:
